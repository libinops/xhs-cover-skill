...
当输入是 Markdown 时，模板会渲染以下元素，样式保持牛皮纸调性：

| 元素 | 字号 | 说明 |
|---|---|---|
| `h1` | 32px | 粗体，墨色，下方 16px 间距 |
| `h2` | 28px | 粗体，墨色，下方 16px 间距 |
| `h3` | 24px | 粗体，墨色，下方 16px 间距 |
| `strong` | 22px（继承） | 粗体 |
| `em` | 22px（继承） | 斜体 |
| `code`（行内） | 20px | 等宽字体，浅墨色背景圆角 |
| `pre code`（代码块） | 16px | 等宽字体，浅墨色背景块，圆角 |
| `ul` / `ol` | 22px（继承） | 左缩进 28px，项间距 6px |
| `blockquote` | 22px（继承） | 左侧 3px 暖灰竖线，文字斜体 |
| `a` | 22px（继承） | 墨色下划线（暖灰下划线颜色） |
| `hr` | —— | 暖灰虚线 |

Markdown 元素仍受 `.body` 的字号 / 行高 / 颜色约束，节奏与纯文本段落一致。
