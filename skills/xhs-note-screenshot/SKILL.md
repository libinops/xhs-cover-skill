...
description: 根据输入文本生成小红书风格的笔记截图（原生帖子观感）。当用户希望得到一张白底、纯正文、模拟真实小红书笔记的卡片时使用：头像 + 作者 + 日期仅出现在第一张图上，后续图片只有正文。头像和作者数据优先取自用户本次提供的链接，其次取自已配置的用户档案。
---

# 小红书笔记截图

本 skill 用于生成看起来像真实小红书笔记帖子的图片卡片 —— 纯白底，没有装饰模板，只有头像 / 作者信息和正文。它与 `lieflat-xhs-longform` 和 `lieflat-xhs-cover` 刻意保持不同：后两者产出的是带样式、带装饰的卡片；本 skill 产出的是原生帖子的观感。
...
- 输出一眼就是真实小红书笔记：白底、没有边框、没有阴影、没有模板装饰。
...
- 日期：`14px`（CSS）→ `28px`（PNG），颜色 `#999999`。
