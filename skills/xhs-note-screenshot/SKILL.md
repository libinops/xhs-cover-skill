...
依赖（`marked`、`playwright`）已经在 `package.json` 中声明。首次使用前，**在 skill 目录内**执行：

```bash
cd skills/xhs-note-screenshot
npm install
npx playwright install chromium   # 仅首次需要安装浏览器
```

后续在该目录下 `npm test` 可跑全部测试（头像解析 + Markdown 渲染）。
...
输入文件可以是纯文本，也可以是 Markdown。脚本使用 [marked](https://marked.js.org/) 解析（GFM 模式，单换行转 `<br>`），支持的元素：

- 标题：`#` / `##` / `###` —— 渲染为 `h1` / `h2` / `h3`
- 强调：`**粗体**`、`*斜体*` —— 渲染为 `strong` / `em`
- 行内代码：`` `code` ``、代码块：`` ```code``` `` —— 渲染为 `code` / `pre code`
- 列表：`- item` / `1. item` —— 渲染为 `ul` / `ol`
- 引用：`> quote` —— 渲染为 `blockquote`（左侧细线）
- 链接：`[text](url)` —— 渲染为 `a`
- 分割线：独占一行的 `---` —— **作为分页信号**（每条 `---` 强制分到新图）

## 分页规则

- **显式分页**：如果输入中包含独占一行的 `---`，每条 `---` 把内容切到下一张图。
- **预算分页**：没有 `---` 时，按空行拆成段落块，再用字符预算（`--max-chars-per-image`，默认 500）分组。
- 第一张图因为带作者头，预算自动 ×0.75。
...
3. 把输入按 Markdown 规则拆分：如果包含独占一行的 `---`，按它强制分页；否则按空行拆成段落块，再用字符预算（`--max-chars-per-image`，默认 500）分组。
4. 每个场景内的 Markdown 块合并后用 `marked` 一次解析成 HTML。
5. 用 `assets/templates/xhs-note-with-author/zh.html` 生成第一张图的 HTML。
6. 用 `assets/templates/xhs-note-body/zh.html` 生成每张正文图的 HTML。
7. 运行轮播导出脚本，得到每张图对应的 PNG。
