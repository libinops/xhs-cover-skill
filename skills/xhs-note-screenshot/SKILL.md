...
本 skill 用于生成看起来像真实小红书笔记帖子的图片卡片 —— 柔和的牛皮纸底色（`#f3e8d2`），没有装饰模板，只有头像 / 作者信息和正文。它与 `lieflat-xhs-longform` 和 `lieflat-xhs-cover` 刻意保持不同：后两者产出的是带样式、带装饰的卡片；本 skill 产出的是原生帖子的观感。
